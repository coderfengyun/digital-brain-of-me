...
**示例**：实现带说话人标注的语音转录，拆为两个组件——A 做说话人识别+时间切片，B 做语音转文字。协作方式是"A 切出时间片段，B 转录对应片段并标注归属"。这个配合方式隐含了一个要求：B 的输出必须携带字级/段级时间戳，否则无法与 A 的切片对齐。如果不先设计协作方式就动手，集成时才发现 B 不支持时间戳，前面的工作就白费了。
