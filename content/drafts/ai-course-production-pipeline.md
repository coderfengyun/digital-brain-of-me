...
一个知识主题的完整课程资料——教学流程、板书内容、教具定义（含数学模型和交互协议）、prompt、UI 场景描述——本质上就是一个代码库。它有明确的目标（教学目标），有多个模块（各环节的内容），模块之间有依赖关系和接口约定。这个"项目"展开为结构化文件后远小于 1 万行，而当前 Agent 已经能稳定处理万行级别的代码项目，能力上是有富裕的。
...
所以结论是：瓶颈不在 AI 的生成能力上，而在两件事——如何设计好 Agent 的 skill 拆分和编排让生成质量足够高，以及如何为课程资料构建更客观的反馈机制让验收收敛足够快。

## 这个方案成立的前提

坦诚地说，这个流程能跑通有几个前提条件：

1. **AI 的生成质量要够用。** 如前面规模和结构的分析，生成能力本身应该不是瓶颈。但草稿需要覆盖所有必要的要素——教学流程、教具定义、prompt 等——且质量足以让课设老师做出"方向对不对"的判断。如果 AI 生成的东西太粗糙，课设老师没法验收，阶段一就会反复迭代，抵消掉并行带来的收益。

2. **局部串行依赖不能拖垮整体并行。** 如前所述，部分角色之间存在先后依赖。需要在实际执行中验证这些局部串行是否会成为新的瓶颈。

3. **验收收敛要够快。** 如前面验证维度的分析，课程资料缺少代码那样的客观反馈机制，验收高度依赖人的主观判断。如果课设老师需要反复让 AI 重新生成才能达到满意，阶段一本身可能就变成了一个耗时的环节。这取决于两件事：课设老师与 AI 协作的熟练度，以及能否逐步构建出更客观的自动化反馈机制来加速收敛。
...
- Jenny Wen（Anthropic Claude 设计负责人）在 [Lenny's Podcast 访谈](https://www.youtube.com/watch?v=eh8bcBIAAFo)中描述了 Anthropic 内部已在实践的相同理念：工程师/AI 先做出粗糙版本，设计师作为顾问角色给反馈，而非先画设计稿再交付。她称之为设计流程被工程速度"逼死"后的新常态。（[中文整理](https://x.com/dotey/status/2028599757820613086)）
